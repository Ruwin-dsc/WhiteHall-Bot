...
exports.help = {
  name: "vc",
  category: 'infos',
  description: 'Permet de voir les stats vocales du serveur',
  utilisation: 'vc',
  permission: 'EVERYONE',
  aliases: ['vcstats', 'vocalstats']
};

exports.run = async (bot, message, args, db) => {
  const voiceChannels = message.guild.channels.cache.filter(channel => channel.type === Discord.ChannelType.GuildVoice);
  ...
  voiceChannels.forEach(channel => {
    ...
    const video = message.guild.members.cache.filter(m => m.voice.selfVideo).size
