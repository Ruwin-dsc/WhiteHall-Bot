...
module.exports = {
    ...
    async execute(role, bot) {
        bot.db.query(`SELECT * FROM logs WHERE guildID = "${role.guild.id}"`, async (err, req) => {
            ...
            if (!roleDeleteEntry) return;

            const { executor, target } = roleDeleteEntry;
            ...
            const embed = new Discord.EmbedBuilder()
                .setTitle(`Rôle Supprimé ! ${bot.emoji.wl}`)
                .setDescription(`> <@${executor.id}> vient de \`supprimer un rôle\` !\n> Rôle : (\`${role.id}\`) (\`${role.name}\`)`)
                .setTimestamp()
                .setColor(bot.color)
